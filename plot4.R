...
epcd4 <- data.table(strptime(epcd1$Day, format = "%d/%m/%Y %H:%M:%S"),
...
epcd4 <- rename(epcd4, Day = V1)
epcd4 <- rename(epcd4, Global_active_power = V2,Global_reactive_power = V3)
epcd4 <- rename(epcd4, Voltage = V4,Global_intensity = V5)
epcd4 <- rename(epcd4, Sub_metering_1 = V6,Sub_metering_2 = V7, Sub_metering_3 = V8)

## plot multiple base plots
par(mfrow = c(2,2),mar = c(5,4,1,6),oma = c(0,0,1,0))
with (epcd4,{
  plot(epcd4$Day,epcd4$Global_active_power,type = "l", xlab = "",ylab = "Global Active Power")
  plot(epcd4$Day,epcd4$Voltage,type = "l", xlab = "datetime",ylab = "Voltage")
  plot(epcd4$Day,epcd4$Sub_metering_1,col="gray", 
       type = "l", xlab = "",ylab = "Energy sub metering")
  points(epcd4$Day,epcd4$Sub_metering_2,col="red",type = "l")
  points(epcd4$Day,epcd4$Sub_metering_3,col="blue",type = "l")
  legend("topright",lty=1,lwd=1,cex=.6,col=c("gray","red","blue"),legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
  plot(epcd4$Day,epcd4$Global_reactive_power,type = "l", xlab = "datetime",ylab = "Global_reactive_power")
  })
